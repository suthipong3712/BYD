...
const PARTS_LABEL = { not_ordered: 'ยังไม่สั่ง', ordered: 'รออะไหล่', arrived: 'มาถึงแล้ว' }
...
function ImportPage({ token }) {
  ...
  function handleFile(e) {
    ...
    authFetch('/admin/import/preview', token, { method: 'POST', body: formData })
      .then((res) => res.json())
      .then((data) => {
        setRows(data.rows)
        setLoading(false)
    ...
    <div className="admin-panel">
      <div className="admin-section">
        ...
        <p className="intake-hint">
          รองรับไฟล์ที่มี 2 ชีต: ชื่อชีตมีคำว่า "รอดำเนินการ" และ "เสร็จสิ้น" — ระบบจะแปลงข้อมูลให้ดูก่อน ยังไม่บันทึกจนกว่าจะกดยืนยัน
        </p>
      ...
      {rows !== null && (
        <div className="admin-section">
          <h2>ตรวจสอบก่อนนำเข้า ({willImport} จะนำเข้า, {willSkip} ข้าม)</h2>
                ...
                {rows.map((r, index) => (
                  <tr key={index} style={{ opacity: r.skip ? 0.4 : 1 }}>
                    <td>
                      ...
                      />
                      {r.skip_reason && (
                        <div style={{ fontSize: '0.7rem', color: 'var(--status-red-text)' }}>
                          {r.skip_reason}
                        </div>
                      )}
                    </td>
                    ...
                    <td>{r.model ?? '-'}</td>
                    <td>{r.job_type === 'warranty' ? 'Warranty' : 'Customer pay'}</td>
          ...
          <button className="intake-submit" onClick={handleCommit} disabled={loading || willImport === 0}>
