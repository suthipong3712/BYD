...
function emptyItem() {
  return { description: '', part_name: '', part_number: '', repair_time_estimate: '', technician_id: '' }
}

function NewOrderForm({ token, vehicleId, onCreated, onCancel }) {
  const [order, setOrder] = useState({ job_type: 'warranty', diagnosis_result: '' })
      ...
      <form onSubmit={handleSubmit}>
        <div className="intake-grid">
          ...
          <input
            ...
            onChange={(e) => setOrder({ ...order, diagnosis_result: e.target.value })}
          />
        </div>
        ...
        {items.map((item, index) => (
          <div className="intake-item-row" key={index}>
            ...
            <input
              ...
              onChange={(e) => updateItem(index, 'repair_time_estimate', e.target.value)}
            />
            <select
        ...
        <button type="button" onClick={addItem}>+ เพิ่มรายการซ่อม</button>

        <div className="new-order-form__actions">
          <button type="submit" className="intake-submit">บันทึกงานใหม่</button>
